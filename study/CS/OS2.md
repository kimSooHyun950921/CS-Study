...
보조기억장치의 공간을 메모리 공간으로 활용한다
이때 cpu는 논리 주소를 갖고 있고, 이를 mmu(HW)가 들고있는 기준 주소와 더해서 실제 메모리 주소를 알아낸다
mmu에서는 context switching을 통해 기준 주소값을 변경한다
```

### 5) **요구 페이징 기법**
```
필요한 페이지만 메모리에 적재하는 기술
여기서는 주소 공간을 여러 페이지로 나눔
이때 valid비트를 활용하여 캐싱을 수행
```
